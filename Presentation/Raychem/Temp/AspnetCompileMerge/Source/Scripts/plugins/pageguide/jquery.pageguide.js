    ...
    var PageGuide = function(guide, options) {
        ...
        if (defaultGuide) {
            if (this.status === "ready") {
                ...
                this.$wrapper.one("ready.pageguide",
                    $.proxy(function() {
                            this.load(defaultGuide);
                        },
                        this));
    ...
    $.extend(PageGuide,
    {
        ...
        prototype: {
            ...
            init: function() {
                ...
                toggle.append("page guide")
                    .append('<div><span class="pageguide-tourtitle"></span></div>')
                    .append('<a class="pageguide-close" title="Close Guide">close guide &raquo;</a>');
                message.append('<a class="pageguide-close" title="Close Guide">close</a>')
                    .append('<span class="pageguide-index"></span>')
                    .append('<div class="pageguide-content"></div>')
                    .append('<a class="pageguide-back" title="Previous">Previous</a>')
                    .append('<a class="pageguide-fwd" title="Next">Next</a>');
            ...
            _registerWrapperHandlers: function() {
                ...
                this.$shadow.on("animationend webkitAnimationEnd oAnimationEnd MSAnimationEnd",
            ...
            _removeCustomStepHandlers: function() {
                $.each(this.$allItems,
                    function(i) {
                        $(this)
                            .off("click.pageguide, show.pageguide, hide.pageguide, select.pageguide, deselect.pageguide");
                    });
            },
            ...
            _scrollIntoView: function(elem) {
                var $t = $(elem).data("tourtarget") ? $($(elem).data("tourtarget")) : $(elem),
                    ...
                    eltop = $t.offset().top,
                    elbtm = eltop + elh,
                    mgn = $(elem).data("options")
                        ? $.extend({}, this.settings.step.margin, $(elem).data("options").margin || {})
                        : this.settings.step.margin,
                    mgnb = Math.max(mgn.bottom, msgh + 15);

                var scrollTo = ((eltop <= dvtop + mgn.top) || (elh > (dvh - mgnb)))
                    ? eltop - mgn.top
                    : (elbtm - (dvh - mgnb));
