    ...
    $.fn.rowGrouping = function(options) {
        ...
        return this.each(function(index, elem) {
            ...
            function fnCreateGroupRow(sGroupCleaned, sGroup, iColspan) {
                ...
                nCell.innerHTML = properties.sGroupLabelPrefix +
                    properties.fnGroupLabelFormat(sGroup == "" ? properties.sEmptyGroupLabel : sGroup, oGroup);
            ...
            function _fnCreateGroup2Row(sGroup2, sGroupLabel, iColspan, oParentGroup) {
                ...
                nCell2.innerHTML = properties.sGroupLabelPrefix2 +
                    properties.fnGroupLabelFormat2(sGroupLabel == "" ? properties.sEmptyGroupLabel : sGroupLabel,
                        oGroup);
            ...
            function _fnGetCleanedGroup(sGroup) {
                ...
                return sGroup
                    .toLowerCase()
                    .replace(/[^a-zA-Z0-9\u0080-\uFFFF]+/g, "-"); //fix for unicode characters (Issue 23)
                ...
                if (properties.bExpandSingleGroup) {
                    if (!bIsExpanded) {
                        ...
                        if (properties.iExpandGroupOffset != -1) {
                            var position = $("#group-id-" + oTable.attr("id") + "_" + sGroup).offset().top -
                                properties.iExpandGroupOffset;
...
//                          sGroupData = oSettings.aoData[oSettings.aiDisplay[iDisplayIndex]]._aData[oSettings.aoColumns[properties.iGroupingColumnIndex].mDataProp];
                        ...
                        if (bUseSecondaryGrouping) {
                            sGroupData2 = oSettings.aoData[oSettings.aiDisplay[iDisplayIndex]]
                                ._aData[properties.iGroupingColumnIndex2];
                            if (sGroupData2 == undefined)
                                sGroupData2 = oSettings.aoData[oSettings.aiDisplay[iDisplayIndex]]
                                    ._aData[oSettings.aoColumns[properties.iGroupingColumnIndex2].mDataProp];
...
// new group encountered (or first of group)
                            ...
                            */
                            if (properties.bAddAllGroupsAsExpanded &&
                                jQuery.inArray(sGroupCleaned, asExpandedGroups) == -1)
                                asExpandedGroups.push(sGroupCleaned);
                        ...
                        if (bUseSecondaryGrouping) {
                            ...
                            $(nTrs[i])
                                .attr("data-group", oGroup2.dataGroup)
            ...
            iYearLength = properties.sDateFormat.toLowerCase().lastIndexOf("y") -
                properties.sDateFormat.toLowerCase().indexOf("y") +
                1;

            var iMonthIndex = properties.sDateFormat.toLowerCase().indexOf("mm");
            var iMonthLength = properties.sDateFormat.toLowerCase().lastIndexOf("m") -
                properties.sDateFormat.toLowerCase().indexOf("m") +
                1;
