...
namespace Corno.Raychem.CustomerPortal.Areas.Wallet.Controllers
{
    ...
    {
        #region -- Constructors -- 
        public CardTransactionController(IWalletService walletService, IdentityManager identityManager)
        {
               ...
               // model.CyberPlatTransId = "JW" + new Guid().ToString();
                switch (submitType)
                {
                    case "LoadWallet":
                        ...
                        returnInfo = _walletService.CardCredit(model);
                       // _prePaidServices.ReflectTransaction(model);
                        successString = "Amount of Rs." + model.Amount + "/- credited to user (" + user.UserName + ") prepaid card successfully, against " + model.BookingId;
                        break;
                    case "RevokeWallet":
                        model.Service = ServiceConstants.Debit;
                        returnInfo = _walletService.CardDebit(model);
                        //_prePaidServices.ReflectTransaction(model);
                        successString = "Amount of Rs." + model.Amount + "/- debited from user (" + user.UserName + ") prepaid card successfully, against " + model.BookingId;
                        break;
                }
                if (null == returnInfo)
