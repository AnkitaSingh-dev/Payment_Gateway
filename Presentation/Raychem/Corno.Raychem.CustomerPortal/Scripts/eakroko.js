    ...
    $.fn.retina = function(retina_part) {
        ...
        if (window.devicePixelRatio >= 2) {
            this.each(function(index, element) {
                ...
                var new_image_src = $(element)
                    .attr("src")
                    .replace(/(.+)(\.\w{3,4})$/, "$1" + settings["retina_part"] + "$2");
                $.ajax({
                    url: new_image_src,
                    type: "HEAD",
                    success: function() {
            ...
            $(".complexify-me")
                .complexify(function(valid, complexity) {
                    ...
                    $(this)
                        .parent()
                        .find(".progress .bar")
                        .width(Math.floor(complexity) + "%")
                        .html(Math.floor(complexity) + "%");
                });
        }
                    ...
                    if ($el.hasClass("filetree-callbacks")) {
                        opt.onActivate = function(node) {
                            ...
                            $(".additionalInformation")
                                .html("<ul style='margin-bottom:0;'><li>Key: " +
                                    node.data.key +
                                    "</li><li>is folder: " +
                                    node.data.isFolder +
                                    "</li></ul>");
            ...
            $(".plupload")
                .each(function() {
                    ...
                    if ($el.hasClass("pl-sidebar")) {
                        ...
                        upload.bind("QueueChanged",
                            function(up) {
                                $(".plupload_droptext").html("<span>Drop files to upload</span>");
                            });
                        upload.bind("StateChanged",
                            function(up) {
                        ...
                        $(".plupload_button")
                            .each(function() {
                                if ($(this).hasClass("plupload_add")) {
                                    $(this)
                                        .attr("class", "btn pl_add btn-primary")
                                        .html("<i class='icon-plus-sign'></i> " + $(this).html());
                                } else {
                                    $(this)
                                        .attr("class", "btn pl_start btn-success")
                                        .html("<i class='icon-cloud-upload'></i> " + $(this).html());
                                }
                            });
                    }
                });
                    ...
                    if (!$(this).hasClass("dataTable-custom")) {
                        var opt = {
                            ...
                            "oLanguage": {
                                ...
                                "sInfo":
                                    "Showing <span>_START_</span> to <span>_END_</span> of <span>_TOTAL_</span> entries",
                        ...
                        if ($(this).hasClass("dataTable-tools")) {
                            opt.sDom = "T" + opt.sDom;
                            opt.oTableTools = {
                                "sSwfPath": "../scripts/plugins/datatable/swf/copy_csv_xls_pdf.swf"
                            };
                        }
